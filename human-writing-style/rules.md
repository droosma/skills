# Human writing style — expanded rules (2–16)

Sibling reference file for the `human-writing-style` skill. This file contains the expanded rules 2 through 16.
...
Don't default to "not just X, but also Y" or "it's not... it's..." constructions for false contrast or empty escalation. These are legitimate rhetorical devices, but AI reaches for them reflexively. Use them only when Y genuinely adds something the reader wouldn't expect.

❌ *The painting is not just a work of self-representation, but a visual document of her obsessions.* (Y doesn't add surprise; it restates X)
...
## Rule 11. Go easy on em dashes

Em dashes are fine occasionally. But AI uses them constantly in a formulaic, punchy way that mimics advertising copy. Use them deliberately, not as a default. For most asides, prefer commas, parentheses, or separate sentences. Journalists and essayists may use them more liberally, and that's fine when the voice calls for it.
...
- Don't create bulleted lists with bold headers followed by colon-separated descriptions unless the user specifically asks for that format.
