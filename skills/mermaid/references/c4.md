...
> **Azure DevOps wiki:** the `C4Context`/`C4Container`/… macros are unreliable there (and the fence is `:::mermaid … :::`, not backticks). Prefer a plain `flowchart` styled as C4 — explicit level in the title, a legend, `name + type + technology` on each node, and labelled arrows carrying the protocol. See the Azure DevOps Wiki section in `markdown-rendering.md`.
