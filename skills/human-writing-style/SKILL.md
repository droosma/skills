...
description: >
  ...
  Also do NOT apply to code-adjacent artifacts (commit messages, PR descriptions, code comments,
  changelogs) unless the user asks for it there, or to prompts, tool descriptions, and other
  text an agent will parse.
...
**Controlled English underneath.** The flavored-mode rules in write-for-audience's `controlled-english.md` apply here too: active voice, simple tenses, no phrasal verbs, no semicolons, no nominalizations, and no hedge dropped when rewriting someone else's text. This skill governs voice and rhythm on top of them. Text an agent will parse uses that file's strict mode instead of this skill.
...
## Narrative shape for fiction and story-like prose

Word choice and sentence rhythm are only part of the problem. AI fiction also
shows recurring patterns in plot, revelation, character agency, time, and
closure. The StoryScope study found that narrative features alone separated
human and AI stories with 93.2% macro-F1 on its test set.

Apply this section to fiction and prose that uses storytelling, such as
anecdotes, case studies, narrative essays, and scene-based openings. Do not
apply it mechanically to technical documentation, reference material, ordinary
emails, or other writing where clarity and completeness matter more than
narrative tension.

Watch for these defaults:

- The narrator explains the theme or moral after the scene already conveys it.
- Every conflict closes cleanly and every question receives an answer.
- Events follow one smooth causal and chronological track.
- Tension rises at a uniform rate toward a conventional climax.
- A revelation supplies new information but does not change how earlier events
  are understood.
- Character motives, emotions, and transformations are fully explained.
- References to books, myths, or history stay vague when a specific reference
  would fit.
- Emotion repeatedly appears through bodily sensations or weather and setting
  metaphors.

Treat these as prompts for review, not bans. A clear chronology, explicit
theme, or resolved ending may be exactly right for the piece. Do not add random
flashbacks, ambiguity, or loose ends as camouflage. Choose the narrative shape
that fits the story and vary it intentionally.

When rewriting existing work, preserve the user's events, claims, and intended
meaning unless they asked for structural revision. See
`references/storyscope.md` for the evidence, scope limits, and detailed
...
**Negative parallelisms:** "This isn't X. This is Y." / "Not X. Y." / "It's not about X. It's about Y." / "Forget X. This is Y." / "Less X, more Y." Any sentence that negates one framing then asserts a corrected one. This is the most reliable single tell of AI text. The fix: delete everything before the positive claim. See rule 9 in `rules.md` for the full pattern list.
...
9. **Negative parallelisms.** Any sentence that negates one framing then asserts a corrected one? ("This isn't X. This is Y." / "It's not about X. It's about Y.") Delete everything before the positive claim.
10. **Triple rhythm.** Are your lists always in threes? Vary the count.
11. **Em dashes.** Any em dashes at all? Replace with commas, periods, or colons. Don't swap in parentheses (that just trades one tell for another), and avoid en dashes or hyphens used as dashes. Never use em dashes.
...
35. **Theme over-explained (narrative prose only).** Does the narrator state the lesson after the scene has already made it clear? Trust the scene or make the explanation earn its place.
36. **Closure too complete (narrative prose only).** Does every conflict, relationship, and question resolve at once? Keep only the closure the piece needs.
37. **Uniform escalation (narrative prose only).** Does tension rise in a smooth staircase? Let intensity pause, reverse, or arrive unevenly when the story supports it.
38. **Single-track time (narrative prose only).** Did chronology become the default rather than a choice? Consider summary, omission, memory, or a shifted frame, but never add temporal tricks without a reason.
39. **Shallow revelation (narrative prose only).** Does a reveal merely answer a question? Stronger revelations can change the meaning of an earlier detail or decision.
40. **Exhaustive interior explanation (narrative prose only).** Are motives and emotions explained past the point where action, dialogue, or a concrete detail already shows them?

If checks 1-34 need deeper guidance, look up the corresponding rule in
`rules.md`. For checks 35-40, use `references/storyscope.md`.
...
**Creative writing:** Apply the narrative-shape review as well as the sentence-level rules. Break any rule if the voice or story calls for it, but break it *deliberately*, not because you defaulted to AI patterns.
...
**Frequency guidance:**
- **HARD RULE:** Never violate. Negative parallelisms, engagement bait, no em dashes. Banned words are avoided unless one is the precise term (see "When a banned word is the right word" above).
...
- **`references/storyscope.md`**: Research-backed narrative tendencies in AI fiction, their scope limits, and ways to use them without forcing artificial complexity. **Read when** drafting or revising fiction, narrative essays, anecdotes, or story-led openings.
